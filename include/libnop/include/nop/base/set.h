...
struct Encoding<std::set<Key, Compare, Allocator>> : EncodingIO<std::set<Key, Compare, Allocator>> {
    ...
    static constexpr std::size_t Size(const Type& value) {
        return BaseEncodingSize(Prefix(value)) + Encoding<SizeType>::Size(value.size()) +
               std::accumulate(value.cbegin(), value.cend(), 0U, [](const std::size_t& sum, const Key& element) {
